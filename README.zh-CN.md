...
1. 打开 ComfyUI，启动 **ComfyUI-Manager**。
2. 搜索 **Aholo World Generate**（发布者：`manycoretech`）。
3. 安装后重启 ComfyUI。

Registry 页面：https://comfyregistry.org/nodes/manycoretech/aholo-world-generate
...
`examples/` 目录提供可导入的 Workflow。在 ComfyUI 中打开 UI workflow：
...
| 文件 | 说明 |
|------|------|
| `spatial_gen_prompt.json` | 纯 prompt |
